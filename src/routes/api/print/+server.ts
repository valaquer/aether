...
export const POST: RequestHandler = async ({ request }) => {
	...
	try {
		execSync(`${PANDOC} "${mdPath}" -f markdown-raw_tex-tex_math_single_backslash-tex_math_dollars -o "${pdfPath}" --pdf-engine=xelatex -V mainfont="Courier New" -V fontsize=8pt -V geometry:"top=0.5in, bottom=0.5in, left=1in, right=1in"`, {
